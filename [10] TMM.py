""" 
TMM grapher for "Tunable Filters for Visible Light Based on Resonant VO2 Planar Thin Films"
Layout: Substrate (Au) --> Si VO2 --> Air
Graph has different refractive index from the paper, but has similar characteristic.
Try checking in 
"""
...
""" 
# Layer's thickness
wl = 632.8		# Reference wavelength (nm)	[input]
tSi = 100		# [input]
tVO2 = 75		# [input]

# choose either T or A
npts = 600 # Number of plotted points		# [input]
wl_low = 400	# lower wavelength limit
wl_high = 1000	# upper wavelength limit
...
nSi = np.interp(wavelengths, np.asarray(r2[0])*1000.0, np.asarray(r2[1]-1j*np.asarray(r2[2])))	# nAl2O3 (Sapphire)
...
"""
nfc = np.concatenate((nSi, ncVO2))	# Cavity structure in cold VO2 phase
nfh = np.concatenate((nSi, nhVO2))	# Cavity structure in hot VO2 phase
...
p1, = ax1.plot(wavelengths, Rc_dB, color="purple", label="cold VO2 (resonant)")
p2, = ax1.plot(wavelengths, Rh_dB, color="purple", label="hot VO2 (non-resonant)", linestyle="dashed")
...
ax1.set_ylabel('Reflection (dB)', color="purple")
