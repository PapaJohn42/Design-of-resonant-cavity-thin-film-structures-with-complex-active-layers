...
#The  files contains a three columns data: wavelength (in um), Real index and Img index
arr = []
f = open('Refractive Index data/nH.data', 'r', encoding='utf-8-sig')
next(f)
for line in f:
...
arr = []	#reset array
f = open('Refractive Index data/nL.data', 'r', encoding='utf-8-sig')
next(f)
for line in f:
	arr.append(map(float, line.split()))
rL = list(zip(*arr))
...
arr = []	#reset array
f = open('Refractive Index data/nC.data', 'r', encoding='utf-8-sig')
next(f)
for line in f:
	arr.append(map(float, line.split()))
rC = list(zip(*arr))
...
arr = []	#reset array
f = open('Refractive Index data/nD.data', 'r', encoding='utf-8-sig')
next(f)
for line in f:
	arr.append(map(float, line.split()))
rD = list(zip(*arr))
...
arr = []	#reset array
f = open('Refractive Index data/na.data', 'r', encoding='utf-8-sig')
next(f)
for line in f:
	arr.append(map(float, line.split()))
ra = list(zip(*arr))
...
nf = np.concatenate((nH, nH,nL,nH, nH,nL,nH, nC,nD,nC, nH,nL,nH, nH,nL,nH, nH,nL,nH, nH,nL,nH, nH))
thk_std = 1550.0/4

thk = np.array([thk_std/2, thk_std,thk_std,thk_std, thk_std,thk_std,thk_std, 32.74,89.86,32.74, thk_std,thk_std,thk_std, thk_std,thk_std,thk_std, thk_std,thk_std,thk_std, thk_std,thk_std,thk_std, thk_std/2 ])	#Thickness of each layer (in nm)
