...
##---From H/2 to U3 '1b'
ns = cr1a[-1] + 1j*ci1a[-1]
(cr1b, ci1b, t1b, qr1b, qi1b) = unit_cell_contour(ns, nH, nL, N, wl, npts)
print('cr1b=', cr1b)
print('ci1b=', ci1b)
...
##---From H/2 to U2 '3b'
ns = cr3a[-1] + 1j*ci3a[-1]
(cr3b, ci3b, t3b, qr3b, qi3b) = unit_cell_contour(ns, -nH, -nL, M, wl, npts)    #-nH and -nL are used since we calculate the coutour in reverse order (mirror of c1)
print('cr3b=', cr3b)
print('ci3b=', ci3b)
...
def two_contour_equations(p, nf1, nf2, ns, na):
	q1,q2 = p
	#print('two contour eq ', q1, q2)        #debug
...
print('q2D=', q2D)
###
(cr2C, ci2C, t2C, qr2C, qi2C) = contour(n1b, nC, q2C, wl, npts)
print('> t2C=', t2C[-1])
print('> q2C=', qr2C[-1]/(np.pi/2) + 1j*qi2C[-1]/(np.pi/2), 'pi/2 unit')
ns=cr2C[-1] +1j*ci2C[-1]
(cr2D, ci2D, t2D, qr2D, qi2D) = contour(ns, nD, q2D, wl, npts)
print('> t2C=', t2D[-1])
print('> q2D=', qr2D[-1]/(np.pi/2) + 1j*qi2D[-1]/(np.pi/2), 'pi/2 unit')
print('> q2C+q2D = ', q2C + q2D)
...
plt.plot(cr2D, ci2D, color='yellow')
