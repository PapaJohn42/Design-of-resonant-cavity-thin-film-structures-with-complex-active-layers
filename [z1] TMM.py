""" 
TMM grapher for "Electrically tunable VO2-metal metasurface for mid-infrared switching, limiting, and nonlinear isolation"
...
""" 
# Layer's thickness
wl = 3900		# Reference wavelength (nm)	[input]
tAu = 70		# [input]
tVO2 = 120		# [input]

npts = 3000 	# Number of plotted points		[input]
y_graph = "T"	# T: transmission or A: absorption	[input]
wl_low = 500		# lower wavelength limit
wl_high = 6000	# upper wavelength limit
...
r2 = dataset_reader("dataset/TiO2.csv")					# Siefke et al. 2016: Thin film; n,k 0.120–125 µm
...
nTiO2 = np.interp(wavelengths, np.asarray(r2[0])*1000.0, np.asarray(r2[1]-1j*np.asarray(r2[2])))	# nAl2O3 (Sapphire)
...
"""
nfc = np.concatenate((ncVO2, nAu))	# Cavity structure in cold VO2 phase
nfh = np.concatenate((nhVO2, nAu))	# Cavity structure in hot VO2 phase
...
ax2.set_ylim(0.0, 0.6)	# Transmission
plt.show()